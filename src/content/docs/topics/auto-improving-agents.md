...
Pruning is first-class, not cleanup. Verbeek's most-emphasized point: the bank goes stale "incredibly quickly." Every model release or feature change rots entries, and stale context actively hampers the agent (context rot). The loop continuously throws knowledge away to stay at the frontier of what's currently solvable; accumulation without measured deletion is the failure mode. Reported effect: stuck and fixing messages down, completion and deploy rate up; all top models in internal rankings consume the Stack Overflow context. This is verified from Lovable production (about 200k projects a day). The discipline runs ahead of the field: mem0's 2026 memory survey still names staleness an open problem and treats deletion as an application-layer decision rather than a native capability, so the broader ecosystem writes memory faster than it decides what to forget. See the note [The Hard Part of a Self-Improving Agent Is Throwing Knowledge Away](/working-intel/notes/lovable-self-improves-every-hour/).

Factory's "Signals" shipped a near-twin of both loops, which marks this as a pattern rather than one team's trick. An LLM judge runs over agent sessions, clusters new friction categories on its own, and files Linear tickets that a Droid agent turns into PRs, with a reported 73% auto-resolving in under four hours. The instructive divergence is privacy: Factory never surfaces raw conversation content (it extracts abstract "facets"), while Lovable pipes the agent's literal venting prose to Slack on the bet that a relatable complaint carries actionable context. Both stop at the same human-approves-the-PR-before-merge line.
...
- [How Lovable self-improves every hour — Benjamin Verbeek](https://www.youtube.com/watch?v=KA5kPbdkK2E) — Benjamin Verbeek, YouTube. Source for holdout-gated knowledge injection ("Lovable Stack Overflow"), blank-injection cohort comparison as the measure of real production lift, aggressive pruning against context rot, stuck-state detection via an LLM judge, and the vent tool. Synthesized in the note [The Hard Part of a Self-Improving Agent Is Throwing Knowledge Away](/working-intel/notes/lovable-self-improves-every-hour/).
- [Signals: Toward a Self-Improving Agent](https://factory.ai/news/factory-signals) — Factory.ai. Source for the convergent friction-detection loop: LLM-judged sessions, embedding-clustered signal categories, auto-filed Linear tickets turned into PRs by an agent (73% auto-resolved under four hours), human approval before merge, and the privacy-by-abstraction "facets" contrast with Lovable's raw vent stream.
- [State of AI Agent Memory 2026](https://mem0.ai/blog/state-of-ai-agent-memory-2026) — mem0. Source for staleness as an open problem in high-relevance memories and pruning/deletion still being an application-layer decision rather than a native memory capability.
...
- **2026-06-21** — Migrated to the public site and run through the publish pipeline. Archived the Lovable talk as ground-truth source; folded the holdout-gate measurement detail and the convergent Factory "Signals" example into the holdout section (see [the note](/working-intel/notes/lovable-self-improves-every-hour/)). Restored Boris Cherny attribution.
- **2026-06-08** — Added tokenomics (the economic gate on always-on agents), corpus-level improvement, and the holdout gate.
- **2026-06-01** — Added production-scale self-referential loops.
- **2026-05-20** — Topic created: the Karpathy triplet, auto-research vs auto-agent, the meta/task split, model empathy, traces as the binding constraint, the local hard takeoff, org-readiness prerequisites, failure modes, and the small-team advantage.
